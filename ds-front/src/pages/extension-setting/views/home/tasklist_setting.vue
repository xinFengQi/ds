...
export default {
  ...
  data() {
    return {
      ...
      giteeDsFlag: "",
      giteeDsPublicFlag: "",
      giteeData: getGiteeObjectKey("codes", "private"),
      giteePublicData: getGiteeObjectKey("codes", "public"),
    };
  },
  mounted() {
    localStorgeData
    ...
    localStorgeData
      .getLocalVariable(getGiteeKey("tasklist", "private_flag"))
      .then((v) => {
        this.giteeDsFlag = v;
      });

    localStorgeData
      .getLocalVariable(getGiteeKey("tasklist", "public_flag"))
      .then((v) => {
  ...
  watch: {
    giteeDsFlag: function (newV, oldV) {
      ...
      localStorgeData.setLocalVariable(
        getGiteeKey("tasklist", "private_flag"),
        newV
      );
    },
    giteeDsPublicFlag: function (newV, oldV) {
      ...
      console.log("存在变化", newV, oldV);
      localStorgeData.setLocalVariable(
        getGiteeKey("tasklist", "public_flag"),
        newV
      );
    },
  },
  methods: {
    giteePrivateOpenChange: function (ev) {
      localStorgeData.setLocalVariable(
        getGiteeKey("tasklist", "private_open"),
        ev
      );
    },
    giteePublicOpenChange: function (ev) {
      localStorgeData.setLocalVariable(
        getGiteeKey("tasklist", "public_open"),
        ev
      );
    },
  },
