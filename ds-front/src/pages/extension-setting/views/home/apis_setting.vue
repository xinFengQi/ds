...
export default {
  ...
  data() {
    return {
      giteeDsFlag: "",
      giteeData: getGiteeObjectKey("apisData", "private"),
      giteeDsPublicFlag: "",
      giteePublicData: getGiteeObjectKey("apisData", "public"),
    };
  },
  mounted() {
    localStorgeData
      .getLocalVariable(getGiteeKey("apisData", "private_open"))
      .then((isShow) => {
        this.giteeMarks_private_open =
          typeof isShow === "string" ? Boolean(isShow) : isShow;
      });
    localStorgeData
      .getLocalVariable(getGiteeKey("apisData", "public_open"))
      .then((isShow) => {
        this.giteeMarks_public_open =
          typeof isShow === "string" ? Boolean(isShow) : isShow;
      });
    localStorgeData
      .getLocalVariable(getGiteeKey("apisData", "private_flag"))
      .then((v) => {
        this.giteeDsFlag = v;
      });
    localStorgeData
      .getLocalVariable(getGiteeKey("apisData", "public_flag"))
      .then((v) => {
  ...
  watch: {
    giteeDsFlag: function (newV, oldV) {
      ...
      localStorgeData.setLocalVariable(getGiteeKey("apisData", "private_flag"), newV);
    },
    giteeDsPublicFlag: function (newV, oldV) {
      ...
      console.log("存在变化", newV, oldV);
      localStorgeData.setLocalVariable(getGiteeKey("apisData", "public_flag"), newV);
    },
  },
  methods: {
    ...
    giteePrivateOpenChange: function (ev) {
      localStorgeData.setLocalVariable(getGiteeKey("apisData", "private_open"), ev);
    },
    giteePublicOpenChange: function (ev) {
      localStorgeData.setLocalVariable(getGiteeKey("apisData", "public_open"), ev);
    },
  },
};
</script>
