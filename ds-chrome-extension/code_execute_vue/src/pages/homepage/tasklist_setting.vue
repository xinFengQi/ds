<template>
  <div>
    ...
    <div class="writer_content">
      <a-switch
        v-model:checked="gitee_tasklist_private_open"
        @change="giteePrivateOpenChange($event)"
      />
    </div>
    <div v-if="gitee_tasklist_private_open">
      <a-page-header
        class="page_header"
        title="个人唯一标识"
    ...
    <div class="writer_content">
      <a-switch
        v-model:checked="gitee_tasklist_public_open"
        @change="giteePublicOpenChange($event)"
      />
    </div>

    <div v-if="gitee_tasklist_public_open">
      <a-page-header
        class="page_header"
        title="公共唯一标识"
...
export default {
  ...
  data() {
    return {
      gitee_tasklist_private_open: false,
      gitee_tasklist_public_open: false,
      giteeDsFlag: "",
      giteeDsPublicFlag: "",
      giteeData: {
        accessToken: {
          varName: "__gitee_tasklist_access_token",
        },
        owner: {
          varName: "__gitee_tasklist_owner",
        },
        repo: {
          varName: "__gitee_tasklist_repo",
        },
      },
      giteePublicData: {
        accessToken: {
          varName: "__gitee_tasklist_public_access_token",
        },
        owner: {
          varName: "__gitee_tasklist_public_owner",
        },
        repo: {
          varName: "__gitee_tasklist_public_repo",
        },
      },
    };
  },
  mounted() {
    chromeUtil.getLocalVariable("__gitee_tasklist_private_open").then((isShow) => {
      this.gitee_tasklist_private_open =
        typeof isShow === "string" ? Boolean(isShow) : isShow;
    });
    chromeUtil.getLocalVariable("__gitee_tasklist_public_open").then((isShow) => {
      this.gitee_tasklist_public_open =
        typeof isShow === "string" ? Boolean(isShow) : isShow;
    });
    chromeUtil.getLocalVariable("__gitee_tasklist_ds_flag").then((v) => {
      this.giteeDsFlag = v;
    });

    chromeUtil.getLocalVariable("__gitee_tasklist_ds_pubilc_flag").then((v) => {
  ...
  watch: {
    giteeDsFlag: function (newV, oldV) {
      ...
      chromeUtil.setLocalVariable("__gitee_tasklist_ds_flag", newV);
    },
    giteeDsPublicFlag: function (newV, oldV) {
      ...
      console.log("存在变化", newV, oldV);
      chromeUtil.setLocalVariable("__gitee_tasklist_ds_pubilc_flag", newV);
    },
  },
  methods: {
    ...
    giteePrivateOpenChange: function (ev) {
      chromeUtil.setLocalVariable("__gitee_tasklist_private_open", ev);
    },
    giteePublicOpenChange: function (ev) {
      chromeUtil.setLocalVariable("__gitee_tasklist_public_open", ev);
    },
  },
};
</script>
