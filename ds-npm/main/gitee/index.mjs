...
function giteePull(options) {
    ...
    const distPath = (customConfig && customConfig.gitee && customConfig.gitee.outputGitDist)
        ? customConfig.gitee.outputGitDist : defaultConfig.gitee.outputGitDist;
    // 获取上传到那个文件夹
    const gitPathJson = (customConfig && customConfig.gitee && customConfig.gitee.gitPath)
    ...
    deleteGitPath = deleteGitPath.map(pas => gitPath + '/' + pas);
